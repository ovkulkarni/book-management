...
def generate_receipt_reports():
    ...
    if select_by == "book":
        ...
        receipts = Receipt.select().where(Receipt.invoice_number == request.form.get(
            "receipt-invoice")).order_by(Receipt.date.desc())
    else:
...
def generate_inventory_reports():
    if request.form.get("max_quantity", None):
        books = Book.select().where(
            Book.count <= int(request.form.get("max_quantity"))).order_by(+Book.title)
    else:
...
def generate_purchase_reports():
    ...
    if report_type == "user":
        ...
        purchases = Purchase.select().where(
            Purchase.seller == account).order_by(Purchase.time.desc())
        ...
        purchases = Purchase.select().where(
            Purchase.total > int(request.form.get("min_amount")))
    elif report_type == "maximum_amount":
        purchases = Purchase.select().where(
            Purchase.total < int(request.form.get("max_amount")))
    else:
        purchases = Purchase.select().order_by(Purchase.time.desc())
